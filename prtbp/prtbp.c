...
NOTE: We are only interested in orbits that go around the origin. For some
values of the energy, the orbit has "contractible" loops, which we want to
avoid. For this, we impose that TWO CONSECUTIVE ITERATES do NOT lie both to
the right or to the left of the origin.
