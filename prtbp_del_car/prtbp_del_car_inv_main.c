/*! \file
    \brief Inverse Poincare map of RTBP in Delaunay coordinates, integrating in
Cartesian: main prog
...
#include <rtbp.h>	        // DIM
#include "prtbp_del_car.h"	// section_t, prtbp_del_car_inv

/**
  Inverse Poincare map of RTBP in Delaunay coordinates, but integrating the
  flow in Cartesian.
  ...
  Compute the n-th iterate of the inverse Poincare map \f$P^{-n}(x)\f$ of the
  RTBP. 
  This procedure also computes, as a side product, the integration time
  to intersect the Poincare section "n" times.
  ...
  2. Compute n-th iterate of the inverse Poincare map, \f$P^{-n}(x)\f$.
...
int main( )
{
           ...
           scanf("%le %le %le %le", x_car, x_car+1, x_car+2, x_car+3)==4 )
   {
       ...
       status=prtbp_del_car_inv(mu,sec,n,x_del,x_car,&ti);
       if(status)
       {
          fprintf(stderr, \
