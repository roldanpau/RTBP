...
// 
// \[ B^+(I) = \int_0^{14M\pi} 
//    f(\gamma_h(s)) e^{it(s)} - f(\gamma_p(s)) e^{i(t(s)-\mu\omega_0^+)} ds, \]
// \[ C^+(I) = \int_0^{-14M\pi}
//    f(\gamma_h(s)) e^{it(s)} - f(\gamma_p(s)) e^{i(t(s)-\mu\omega_0^-)} ds. \]
//
...
// (depending if this function is called from re_B or re_C),
// \[ f(\gamma_h(s)) e^{it(s)} - f(\gamma_p(s)) e^{i(t(s)-\mu\omega_0^+)} \]
// \[ f(\gamma_h(s)) e^{it(s)} - f(\gamma_p(s)) e^{i(t(s)-\mu\omega_0^-)} \]
//
// re_B
// im_B
...
#include <frtbpred.h>
#include <inner_ell.h>			// re_f_integrand, im_f_integrand
...
// name OF FUNCTION: re_integrand_B
//
// PURPOSE
// =======
// Given the periodic point $p$ and the homoclinic point $h$ of the circular
...
double re_integrand_B(double s, void *params)
{
   ...
   mu = ((struct iparams_outer_ell *)params)->mu;

   for(i=0;i<DIMRED;i++)
   {
      p[i] = ((struct iparams_outer_ell *)params)->p_red[i];
      h[i] = ((struct iparams_outer_ell *)params)->h_red[i];
   }

   // Compute $\Phi_s(p)$
   status = frtbp_red(mu,s,p);
   if(status)
   {
   ...
   // p2=\Phi_s(p)
   for(i=0;i<DIM;i++) p2[i]=p[i];

   // h2=\Phi_s(h)
   for(i=0;i<DIM;i++) h2[i]=h[i];

   // Compute first term in integrand: f(\gamma_h(s)) e^{it_h} (real part).
   re_fh = re_f_integrand(mu,h2);
   im_fh = im_f_integrand(mu,h2);
   term1 = re_fh*cos(t_h) + im_fh*sin(t_h);

   // Compute second term in integrand: f(\gamma_p(s)) e^{i(t_p-\mu\omega)}
   // (real part). Notice that t_p has already been shifted by -\mu\omega in
   // function re_B, so no need to do it here.
   re_fp = re_f_integrand(mu,p2);
   im_fp = im_f_integrand(mu,p2);
   term2 = re_fp*cos(t_p) + im_fp*sin(t_p);

   return term1 - term2;
}

double im_integrand_B(double s, void *params)
{
   double mu;
   double p[DIMRED];
   double h[DIMRED];
   ...
   // -\mu\omega in function re_B, so no need to do it here.
   re_fp = re_f_integrand(mu,p2);
   im_fp = im_f_integrand(mu,p2);
...
//    - \gamma_p(s) = \Phi_s{l_p,L_p,0,G_p,t_0-\mu\omega_0^+}
//
...
// Equivalently, we split the integral from 0 to 14N\pi into N parts of size
// 14\pi:
//
// \[ B^+(I) = 
//    \Sum_{i=0,N-1}
//       \int_{0}^{14\pi}
//          f(\gamma_h(s)) e^{it(s)} 
//          - f(\gamma_p(s)) e^{i(t(s)-\mu\omega_0^+)} ds,
// \]
//
// where 
//    - \gamma_h(s) = \Phi_{-14M\pi+14i\pi+s}{z_u,t_0+t_f}
//    - \gamma_p(s) = \Phi_{14i\pi+s}{l_p,L_p,0,G_p,t_0-\mu\omega_0^+}
//
...
// \Phi_{14\pi}(l_p,L_p,0,G_p) = (l_p,L_p,0,G_p).
//
// This is computed using numerical integration.
//
// PARAMETERS
...
//    mass parameter for the RTBP
// p
//    p=(l_p,L_p,g_p=0,G_p), periodic point of period 7, on the section g=0.
// zu
      ...
      int M, int N)
{
   ...
   xi_red[4] = 0;              // t_0
   xi_red[5] = 0;              // I_0
   frtbp_red(mu, -14*M*M_PI, xi_red);
   tf = -xi_red[4];

   for(j=0;j<DIM;j++) pi_red[j]=p[j];
   // Initialize t,I components.
   pi_red[4] = -mu*omega;  // t_0-\mu\omega_0^+
   pi_red[5] = 0;              // I_0
   ...
   for(i=0; i<N; i++)
   {
      ...
      for(j=0;j<DIM;j++) xi_red[j]=zu[j];
      xi_red[4] = tf;	// t_0+t_f
      xi_red[5] = 0;	// I_0
      status = frtbp_red(mu,-(M-i)*14*M_PI,xi_red);
      if(status)
      {
	 fprintf(stderr, "re_B: error integrating trajectory");
	 exit(EXIT_FAILURE);
      }

      for(j=0;j<DIMRED;j++)
      {
	 params.p_red[j] = pi_red[j];
	 params.h_red[j] = xi_red[j];
      }

      // Integrate integrand function by parts. 
      // Previously, we used 14M parts of size \pi. Now we use M parts of
      // size 14pi.
      // We request a absolute error of 0 and a relative error RELERROR.
      gsl_integration_qags (&F, 0, 14*M_PI, 0, RELERROR, NINTERVALS, w,
	    &result_i, &error);
      ...
      // \Phi_{14\pi}(l_p,L_p,0,G_p) = (l_p,L_p,0,G_p).
      for(j=0;j<DIM;j++) pi_red[j]=p[j];
      status = frtbp_red(mu,14*M_PI,pi_red);
      if(status)
      {
	 fprintf(stderr, "integrand: error integrating trajectory");
	 exit(EXIT_FAILURE);
...
//
// where 
//    - \gamma_h(s) = \Phi_{14M\pi-14i\pi+s}{z_s,t_0+t_f}
//    - \gamma_p(s) = \Phi_{-14i\pi+s}{l_p,L_p,0,G_p,t_0-\mu\omega_0^-}
//
// Since the periodic orbit is unstable, we escape from it after a long
// integration time. Thus, it is important to exploit the fact that
      ...
      int M, int N)
{
   ...
   for(i=0; i<N; i++)
   {
      ...
      for(j=0;j<DIM;j++) xi_red[j]=zs[j];
      xi_red[4] = tf;	// t_0+t_f
      xi_red[5] = 0;	// I_0
      status = frtbp_red(mu,(M-i)*14*M_PI,xi_red);
      if(status)
      {
	 fprintf(stderr, "re_C: error integrating trajectory");
	 exit(EXIT_FAILURE);
      }
