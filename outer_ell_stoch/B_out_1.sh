...
cut -d ' ' -f 13-16 ../intersec_del_car/intersecs_unst_SECg_br1.res \
   | head -n $lines > temp3
...
cut -d ' ' -f 2 ../approxint_del_car/approxints_unst_SECg_br1.res \
    | head -n $lines > temp4

echo "0.95387536e-3" > $DATFILE

# H, p, z_u, \omega_pos, N
paste -d ' ' temp1 temp2 temp3 ../outer_circ/omega_neg_SECg_br1.res temp4 \
    >> $DATFILE
rm temp*
