...
int tanvec_u(double mu, double H, section_t sec, double v_u[2], int n, double p_u[2], 
      double w[2]);
...
int splitting_del_car_unst(double mu, double H, section_t sec, double v[2], int n, 
        double p[2], double *angle)
{
   double w[2]; // tangent vector to the unstable manifold at z

   // auxiliary variables
   double u[2];     // vertical vector: either (0,1) or (0,-1)
   double alpha;    // splitting half-angle

   tanvec_u(mu, H, sec, v, n, p, w);
   ...
   // alpha = angle from u to w
   alpha=atan2(w[1],w[0]) - atan2(u[1],u[0]);
...
/**
  ...
  Exactly as \ref splitting_del_car_unst.
 */

int splitting_del_car_st(double mu, double H, double v[2], int n, double p[2],
      double *angle)
{
...
int tanvec_u(double mu, double H, section_t sec, double v_u[2], int n, 
        double p_u[2], double w[2])
{
